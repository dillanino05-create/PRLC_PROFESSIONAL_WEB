...
function launchApp() {
  ...
  const savedConfigStr = localStorage.getItem(`mecapsi_config_${u.username}`);
  if (savedConfigStr) {
      try {
...
function navigateTo(section) {
  ...
  setTimeout(() => {
    ...
    switch (section) {
      ...
      case 'config':          html = buildConfig(isAdmin);          break;
...
function attachSectionListeners(section, isAdmin) {
  ...
  qs('#btn-save-config')?.addEventListener('click', () => {
    const theme = qs('#inp-cfg-theme')?.value;
    const notifs = qs('#inp-cfg-notifs')?.value;
    ...
    // Save to localStorage using the current user's username
    if (DB.state.currentUser && DB.state.currentUser.username) {
        const userSettings = {
            theme: theme,
            notifs: notifs
        };
        localStorage.setItem(`mecapsi_config_${DB.state.currentUser.username}`, JSON.stringify(userSettings));
    }
...
function openModalNuevoPaciente() {
  openModal(
    ...
    `<button class="btn-secondary" onclick="closeModal()">Cancelar</button>
     <button class="btn-primary" onclick="closeModal(); toast('✅ Paciente registrado con éxito en el DSS.', 'success');">Guardar Paciente</button>`
  );
}

/* ── Modal: Nueva Cita ── */
window.openModalNuevaCita = function() {
  const opts = DB.pacientes.map(p => `<option value="${p.id}">${p.name}</option>`).join('');
  openModal(
    '📅 Agendar Nueva Cita',
    `<div class="field">
      <label>Paciente</label>
      <select id="inp-cita-pac">${opts}<option value="nuevo">+ Nuevo paciente...</option></select>
    </div>
    <div class="field-row">
      <div class="field">
        <label>Fecha</label>
        <input type="date" id="inp-cita-fec" value="2026-04-18" />
      </div>
      <div class="field">
        <label>Hora</label>
        <input type="time" id="inp-cita-hora" value="10:00" />
      </div>
    </div>
    <div class="field">
      <label>Tipo de Sesión</label>
      <select id="inp-cita-tipo">
        <option value="PLC v3 — Evaluación">PLC v3 — Evaluación (Primera vez)</option>
        <option value="Seguimiento Clínico">Seguimiento Clínico</option>
        <option value="Devolución de resultados">Devolución de resultados</option>
    ...
    <div class="field">
      ...
      <div style="display:flex; gap:10px; margin-top:5px;">
        <label style="display:flex;align-items:center;gap:5px;font-weight:normal;color:var(--text);"><input type="radio" name="mod" checked> Presencial</label>
        <label style="display:flex;align-items:center;gap:5px;font-weight:normal;color:var(--text);"><input type="radio" name="mod"> Virtual (Telemedicina)</label>
      </div>
    </div>
    <div class="dss-note" style="margin-top:var(--sp-2);">ℹ️ La agenda está vinculada a las métricas del dashboard automáticamente.</div>`,
    `<button class="btn-secondary" onclick="closeModal()">Cancelar</button>
  ...
  document.getElementById('btn-save-cita')?.addEventListener('click', () => {
    let pacId = document.getElementById('inp-cita-pac').value;
    let pacName = pacId;

    const pacObj = DB.pacientes.find(p => String(p.id) === String(pacId) || p.name === pacId);
    if(pacObj) pacName = pacObj.name;
    else if(pacId === 'nuevo') pacName = 'Nuevo Paciente (Sin registro)';

    const hora = document.getElementById('inp-cita-hora').value;
    const tipo = document.getElementById('inp-cita-tipo').value;
    ...
    if(DB.state.currentSection === 'agenda' || DB.state.currentSection === 'dashboard') {
      navigateTo(DB.state.currentSection);
    }
  });
}

/* ── Modal: Calendario Completo ── */
window.openModalCalendarioCompleto = function() {
  openModal(
    '📆 Calendario de Abril 2026',
    `<div style="display:grid; grid-template-columns:repeat(7,1fr); gap:10px; text-align:center;">
       ${['L', 'M', 'X', 'J', 'V', 'S', 'D'].map(d => `<div style="font-weight:700;color:var(--text-muted);font-size:.875rem;">${d}</div>`).join('')}
       ${Array.from({length:30}, (_,i) => i+1).map(d => {
         const isToday = d===18;
         return `<div style="padding:15px; background:${isToday ? 'var(--primary-vivid)' : 'var(--surface-high)'}; color:${isToday ? '#fff' : 'var(--text)'}; border-radius:var(--r-sm); font-weight:${isToday ? 700 : 500}; border:${isToday ? 'none' : '1px solid var(--outline-var)'}; cursor:pointer;" onclick="closeModal(); window.openModalNuevaCita && window.openModalNuevaCita()">
             ${d}
           </div>`;
       }).join('')}
     </div>
     <div class="dss-note" style="margin-top:var(--sp-2);">ℹ️ Haz clic en un día para agendar una nueva cita.</div>`,
    `<button class="btn-primary" onclick="closeModal()">Cerrar Calendario</button>`
  );
};
...
setInterval(() => {
  ...
  if (clockEl || dateEl) {
    ...
    if (clockEl) {
      ...
      const p = h >= 12 ? 'PM' : 'AM';
      h = h % 12 || 12;
      clockEl.innerHTML = `${h}:${m} <span style="font-size:1rem;color:var(--text-muted);">${p}</span>`;
    }
    if (dateEl) {
