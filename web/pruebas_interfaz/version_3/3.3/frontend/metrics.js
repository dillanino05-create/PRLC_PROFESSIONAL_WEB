...
function calcMetrics(linesData, clickLog, age) {
  ...
  let sysFocus = [];
  if (CP >= 80) sysFocus.push('Selectiva preservada');
  else if (COM > O) sysFocus.push('Selectiva con falla inhibitoria');
  else sysFocus.push('Selectiva en déficit de rastreo');

  if (TRM >= -10 && estabilidad >= 80) sysFocus.push('Sostenida óptima');
  else if (TRM < -25) sysFocus.push('Sostenida en declive chronológico');
  else sysFocus.push('Sostenida fluctuante');
