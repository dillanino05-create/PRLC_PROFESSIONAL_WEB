...
// Copiado sin modificaciones desde PLC Professional original
...
const STIM_TYPES = {
  ...
  'D13': { is_target:false, sq_left:true,  sq_right:true,  sq_top:false, sq_bottom:false, h_offset:0,   v_asym:-12 },
  'D14': { is_target:false, sq_left:true,  sq_right:true,  sq_top:false, sq_bottom:false, h_offset:0,   v_asym:12  },
};
...
function drawPLCStimulus(ctx, stype, w, h, fg = '#1A1A2E', bg = '#FFFFFF', lw = 1.8) {
  ...
  function sq(x, y) { ctx.fillRect(x - s / 2, y - s / 2, s, s); }
...
function generatePracticeLine(n = 10, mandatoryDists = []) {
  ...
  while (dists.length < n - 2) dists.push(weightedRandom(DISTRACTOR_KEYS, DISTRACTOR_WEIGHTS));
