...
function buildAgenda() {
  ...
  <div class="page-header anim anim-1">
    ...
    <button class="btn-primary" id="btn-add-cita">+ Agendar Cita</button>
  </div>
  ...
  <div class="content-row anim anim-2" style="grid-template-columns:1fr 300px;">
    <article class="card">
      <div class="card-header">
        ...
        <div class="card-sub">${DB.agenda.length} sesiones · Última: ${DB.agenda[DB.agenda.length - 1].time}</div>
      </div>
      <div style="padding:0 var(--sp-2) var(--sp-2);">
        ${DB.agenda.map((a, i) => `
        <div class="agenda-item" id="cita-${i}" style="cursor:pointer;" title="Clic para ver detalle">
          <div class="agenda-time">${a.time}</div>
          <div class="agenda-dot" style="background:${a.color};box-shadow:0 0 8px ${a.color}80;"></div>
          <div class="agenda-info">
            <div class="agenda-name">${a.name}</div>
            <div class="agenda-prueba">${a.tipo}</div>
          </div>
          ${chipHtml(a.estado === 'Confirmada' ? 'dss-p0' : 'dss-p2', a.estado)}
          <button class="btn-mini view btn-cita-detail" data-idx="${i}" style="margin-left:8px;">Ver</button>
        </div>`).join('')}
      </div>
    </article>
    ...
    <div style="display:flex;flex-direction:column;gap:var(--sp-2);">
      ...
      <article class="card" style="padding:var(--sp-3);">
        <div class="card-title" style="margin-bottom:var(--sp-2);">📆 Abril 2026</div>
        ${buildMiniCalendar()}
      </article>
